...
import { MessageSquare, Camera, Phone, Plus, Accessibility, Eye, Type, Image as ImageIcon } from 'lucide-react';
        ...
        <div className="relative flex flex-col items-start">
          <AnimatePresence>
            ...
            scale: 0.8
                ...
                <button onClick={() => accessibility.setHighContrast(!accessibility.highContrast)} className={`flex items-center gap-3 w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors ${accessibility.highContrast ? 'bg-primary text-white' : 'hover:bg-gray-100 text-gray-700'}`}>
                  <Eye className="w-4 h-4" /> Kontras Tinggi
                </button>

                <button onClick={() => accessibility.setLargeText(!accessibility.largeText)} className={`flex items-center gap-3 w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors ${accessibility.largeText ? 'bg-primary text-white' : 'hover:bg-gray-100 text-gray-700'}`}>
                  <Type className="w-4 h-4" /> Teks Besar
                </button>

                <button onClick={() => accessibility.setGrayscale(!accessibility.grayscale)} className={`flex items-center gap-3 w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors ${accessibility.grayscale ? 'bg-primary text-white' : 'hover:bg-gray-100 text-gray-700'}`}>
                  <ImageIcon className="w-4 h-4" /> Grayscale
                </button>
              </motion.div>}
          </AnimatePresence>

          <button onClick={() => setIsAccessibilityExpanded(!isAccessibilityExpanded)} className="w-14 h-14 bg-accent hover:bg-orange-500 text-white rounded-xl flex items-center justify-center shadow-lg transition-transform hover:scale-105 border-2 border-white">
